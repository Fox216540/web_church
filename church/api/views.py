...
from .models import Sermon, Event, Content
from .model_pidantic import SermonSchema, EventSchema, ContentSchema
...
def get_contents(request, category: str = None):
	qs = Content.objects.all()
	if category:
		qs = qs.filter(category=category)
	return [ContentSchema(url=obj.url) for obj in qs]